        ...
        <div class="" id="app">
            <div class="row no-gutters">
                ...
                <div class="col-sm-11 p-0">
                    <div class="step-bar">
                        <step-progress @if(Session::get('locale') == 'ar') :steps="letterStepsArabic" @else :steps="letterSteps" @endif :current-step=6 icon-class="fa fa-check"></step-progress>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="col-sm-12">
                    <div class="content-panel">
                        ...
                        <div class="form-panel pt-2">
                            <div class="row">
                                <div class="col-sm-7">
                                <h2>{{ __('message.letterBodyHeader') }}</h2>
                                ...
                                <form class="" method="POST" action="{{ route('letter.save') }}" autofocus="off">
                                        ...
                                        <div class="col-sm-12">
                                            <div class="form-group">
                                                <div class="floating-input">
                                                    <textarea name="body" id="body" class="form-control ck_editor body @error('body') is-invalid @enderror"
                                                    >@if(!empty($userLetter->body) && isset($userLetter->body)){{ old('body',$userLetter->body) }}
                                                        @else{{ old('body') }}
                                                        @endif</textarea>
                                    ...
                                    <div class="mt-5">
                                        <a class="btn btn-outline-secondary text-center text-uppercase"
                                           ...
                                           @else href="{{ route('letter.opener') }}" @endif>{{__('fields.back') }}</a>
...
@section('scripts')
    <script type="text/javascript">
        $(document).ready(function () {
            var tooltipContent ="{{ __('message.bodyTipsSubTitle') }}"+
                "<ul class='tips-container'>" +
                "<li>{{ __('message.bodyTips1') }}</li>" +
                "<li>{{ __('message.bodyTips2') }}</li>" +
                "<li>{{ __('message.bodyTips3') }}</li>" +
                "<li>{{ __('message.bodyTips4') }}</li>" +
                "<li>{{ __('message.bodyTips5') }}</li>" +
                "</ul>";
            $('#tooltip').popover({
                title: "{{ __('message.bodyTipsTitle') }} <a href='#' class='close' data-dismiss='alert'>&times;</a>",
