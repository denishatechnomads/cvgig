                ...
                <div class="col-sm-11 p-0">
                    <div class="step-bar">
                        <step-progress @if(Session::get('locale') == 'ar') :steps="letterStepsArabic"
                                       @else :steps="letterSteps" @endif :current-step=4
                ...
                <div class="col-sm-12">
                    <div class="content-panel">
                        ...
                        <div class="form-panel pt-2">
                            <div class="row">
                                <div class="col-sm-7">
                                    <h3>{{ __('message.letterGreetingHeader') }}</h3>
                                    ...
                                    <form class="" method="POST" action="{{ route('letter.save') }}" autofocus="off">
                                            ...
                                            <div class="col-sm-12">
                                                <div class="form-group">
                                                    ...
                                                    <textarea name="greeting" id="greeting"
                                                              class="form-control ck_editor greeting @error('greeting') is-invalid @enderror"
                                                    >@if(!empty($userLetter->greeting) && isset($userLetter->greeting)){{ old('greeting',$userLetter->greeting) }}
                                                        @else{{ old('greeting') }}
                                                        @endif</textarea>
                                        ...
                                        <div class="mt-5">
                                            <a class="btn btn-outline-secondary text-center text-uppercase"
                                               ...
                                               @else href="{{ route('letter.subject') }}" @endif>{{__('fields.back') }}</a>
                                            <button type="submit"
...
@section('scripts')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            var tooltipContent = "{{ __('message.greetingTipsSubTitle') }}" +
                "<ul class='tips-container'>" +
                "<li>{{ __('message.greetingTips1') }}</li>" +
                "<li>{{ __('message.greetingTips2') }}</li>" +
                "<li>{{ __('message.greetingTips3') }}</li>" +
                "<li>{{ __('message.greetingTips4') }}</li>" +
                "<li>{{ __('message.greetingTips5') }}</li>" +
                "</ul>";
            $('#tooltip').popover({
                title: "{{ __('message.greetingTipsTitle') }} <a href='#' class='close' data-dismiss='alert'>&times;</a>",
